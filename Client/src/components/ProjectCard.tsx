      ...
      <div className="bg-white/5 border border-white/10 rounded-xl overflow-hidden hover:border-white/20 transition group">
                ...
                >
                  {gen.generatedImage && (
                    <a
                      ...
                      download={`${gen.productName}-post.jpg`}
                      target="_blank"
                      rel="noreferrer"
        ...
        <div className="p-4">
          {forCommunity && gen.user?.name && (
            <div className="flex items-center gap-2 mb-3">
              {gen.user.image ? (
                ...
                <div className="w-7 h-7 rounded-full bg-violet-500/30 flex items-center justify-center text-xs">
                  {gen.user.name.charAt(0)}
                </div>
              )}
              <span className="text-sm text-gray-300">{gen.user.name}</span>
            </div>
          )}
          ...
          {gen.productDescription && (
            <div className="mt-3">
              <p className="text-xs text-grey-400 mb-1">Description</p>
              <div className="text-sm text-grey-300 bg-white/3 p-2 rounded-md wrap-break-word">
                {gen.productDescription}
              </div>
            </div>
          )}
          ...
          {gen.userPrompt && (
            <div className="mt-3">
              <div className="text-sm text-grey-300">{gen.userPrompt}</div>
            </div>
          )}
          {/* buttons */}
          {!forCommunity && (
            <div className="mt-4 grid grid-cols-2 gap-3" ><GhostButton className="text-xs justify-center"
            onClick={() => {navigate(`/result/${gen.id}`); scrollTo(0,0)}}>
              View Details
              </GhostButton>
              <PrimaryButton onClick={() => togglePublish(gen.id)}
                className="rounded-md">
                  {gen.isPublished ? 'Unpublic' : 'Publish'}
              </PrimaryButton>
              </div>
          )}
        </div>
